...
if [ "$RUN_BENCHMARKS" = "true" ]; then
    ...
    for SWEEP_DIR in "$CONFIGS_DIR"/*; do
        if [ -d "$SWEEP_DIR" ]; then
            ...
            # Run configurations in alphabetical order. Filenames are generated
            # with a zero-padded sequence suffix so alphabetical order matches
            # generation order: the build config always runs before any
            # search-only configs that share the same index.
	    for CONFIG_FILE in $(find "$SWEEP_DIR" -name "*.json" | sort); do
