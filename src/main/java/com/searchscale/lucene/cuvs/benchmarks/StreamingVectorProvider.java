...
public class StreamingVectorProvider implements VectorProvider {
  private static final Logger log =
      LoggerFactory.getLogger(StreamingVectorProvider.class.getName());
  ...
  public StreamingVectorProvider(String filePath, int maxVectors) throws IOException {
    ...
    log.info(
        "StreamingVectorProvider initialized: {} vectors, {} dimensions, format: {}",
        vectorCount,
        dimension,
        format);
  }
  ...
  private int countVectorsInCompressedFile(int maxVectors, int dimension, long vectorSize)
      throws IOException {
    int count = 0;
    try (FileInputStream fis = new FileInputStream(filePath);
